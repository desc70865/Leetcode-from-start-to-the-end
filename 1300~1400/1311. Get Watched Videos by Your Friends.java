...
class Solution {
    public List<String> watchedVideosByFriends(List<List<String>> watchedVideos, int[][] friends, int id, int level) {
        ...
        while (! queue.isEmpty()) {
            // System.out.println(watchedVideos.get(queue.peekFirst()));
            for (String s: watchedVideos.get(queue.pollFirst())) {
        ...
        Collections.sort(list, (x, y) -> x.v == y.v ? x.s.compareTo(y.s) : x.v - y.v);
        List<String> res = new ArrayList<>();
        for (Node cur: list) res.add(cur.s);
        return res;
    }
}

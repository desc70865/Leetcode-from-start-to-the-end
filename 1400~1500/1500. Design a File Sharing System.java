...
class FileSharing {
    ...
    LinkedList<List<Integer>> userList;
    public FileSharing(int m) {
        fileList = new List[m + 1];
    ...
    public int join(List<Integer> ownedChunks) {
        ...
        for (;userId < userList.size();userId++) {
            if (userList.get(userId) == null) {
                userList.remove(userId);
                userList.add(userId,new ArrayList<>());
                hasJoin = true;
                break;
            }
        }
    ...
    public void leave(int userID) {
        List<Integer> chunks = userList.remove(userID);
        if (chunks != null) {
            userList.add(userID,null);
            for (Integer i : chunks) {
                fileList[i].remove((Integer)userID);
            }
        }
    }
    
    public List<Integer> request(int userID, int chunkID) {
        List<Integer> chunk = fileList[chunkID];
        List<Integer> ret = chunk != null ? new ArrayList(chunk) : new ArrayList();
        if (chunk != null && chunk.size() > 0) {
            putFile(chunkID,userID);
        }
        ret.sort(null);
        return ret;
    }

    private void putFile(int idx,int userId) {
        if (fileList[idx] == null) {
            fileList[idx] = new ArrayList<>();
        }
        if (!fileList[idx].contains(userId)){
            fileList[idx].add(userId);
            userList.get(userId).add(idx);
        }
    }
}

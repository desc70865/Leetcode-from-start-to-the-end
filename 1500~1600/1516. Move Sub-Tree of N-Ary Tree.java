...
class Solution {
    public Node moveSubTree(Node root, Node p, Node q) {
        ...
        Node pParent = getParent(p, root, dummy);
        if (getParent(q, p, pParent) != null) {
            getParent(q, root, dummy).children.remove(q);
            List<Node> list = pParent.children;
    ...
    private Node getParent(Node target, Node node, Node parent) {
        if (node == null) {
            return null;
        }
        if (node == target) {
        ...
        for (Node next: node.children) {
            Node cur = getParent(target, next, node);
            if (cur != null) {
                return cur;
            }
        }

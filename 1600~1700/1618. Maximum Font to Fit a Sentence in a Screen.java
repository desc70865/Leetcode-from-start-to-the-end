...
class Solution {
    public int maxFont(String text, int w, int h, int[] fonts, FontInfo fontInfo) {
        ...
        while (l <= r) {
            int m = l + r >> 1;
            if (helper(w, h, fontInfo, fonts[m], map)) {
                l = m + 1;
            } else {
                r = m - 1;
            }
    ...
    private boolean helper(int w, int h, FontInfo fontInfo, int size, int[] map) {
        if (fontInfo.getHeight(size) > h) {
            return false;
        }
        for (char c = 'a'; c <= 'z' && w >= 0; c++) {
            w -= fontInfo.getWidth(size, c) * map[c];
        }
        return w >= 0;
    }
}
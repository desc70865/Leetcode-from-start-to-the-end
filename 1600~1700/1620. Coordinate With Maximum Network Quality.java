...
class Solution {
    public int[] bestCoordinate(int[][] towers, int radius) {
        ...
        for (int i = 0; i < N; i++) {
            int cur = 0;
            for (int j = 0; j < N; j++) {
                cur += helper(towers[i], towers[j], radius);
            }
            if (cur > max) {
                max = cur;
                idx = i;
            } else if (cur == max) {
                if (towers[i][0] < towers[idx][0] || towers[i][0] == towers[idx][0] && towers[i][1] < towers[idx][1]) {
                    idx = i;
                }
            }
        }
    ...
    private int helper(int[] A, int[] B, int r) {
        int D = dis(A, B);
        if (D > r * r) return 0;
        return (int) ((double) B[2] / (1 + Math.sqrt(D)));
    }

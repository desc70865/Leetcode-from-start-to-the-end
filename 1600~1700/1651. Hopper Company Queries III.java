...
(SELECT
    ...
    ROUND(AVG(total_time) OVER (ORDER BY month ROWS BETWEEN CURRENT ROW AND 2 FOLLOWING), 2) AS average_ride_duration
FROM
# 计算所有距离和时间
(SELECT
    mon AS month,
...
LEFT JOIN
(SELECT
    MONTH(requested_at) AS date,
    SUM(ride_distance) AS total_distance,
    SUM(ride_duration) AS total_time

...
(
    SELECT *
        , COUNT(*) OVER(PARTITION BY gold_medal) gold_count
    FROM Contests
) C
...
(
    SELECT *
    , ROW_NUMBER() OVER(PARTITION BY medal ORDER BY contest_id) rnk
    FROM A
) D
ON D.medal = U.user_id
GROUP BY user_id,(D.contest_id - rnk)
HAVING COUNT(*) >= 3
;